...
## Demo
[only when /e2e-verify ran for this issue — omit the section entirely for infra-only PRs with no journey.json Goal/Step:
[one line: what you'll see, e.g. "AP controller locks the budget cap and vendor list on-chain via ENS"]
Recording: `[local path /e2e-verify returned, e.g. test-results/g1-ap-controller-locks-rules/video.webm]` (gitignored — not in this diff, pull the branch and open it locally)]
EOF
)"
```
...
- Demo section only appears when `/e2e-verify` actually ran (see `/sprint` Phase 2.5) — never fabricate a recording path, and never claim a demo exists for an issue that skipped the gate as infra-only.
