...
flowchart TD
    ...
    ROUTE -- "create" --> CREATE["Post issue to Agent Insure / Orbbit Tech"]
...
Initiative: `Hackathon` · Project: `Agent Insure` · Team: `Orbbit Tech`
...
| Field | Rule |
|---|---|
| `title` | Names the outcome achieved (e.g. "Wire up claim eligibility check"); must match the spec H1 exactly. The spec filename is derived from this title: `TECH-N-[title-slug].md` (lowercase, hyphenated). |
| `team` | `Orbbit Tech` |
...
| `project` | Always `"Agent Insure"` — pass the name string directly |
...
**Milestone IDs — Agent Insure:**

| Milestone | ID |
|---|---|
| Submit hackathon project | `<TODO: look up via Linear MCP — the OpenPop milestone ID does not apply to this project>` |
