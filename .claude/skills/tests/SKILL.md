...
▶ /tests · awaiting approval
  🏗️ Layer:   Contracts
  📋 Plan:    specs/agent-insure/m1-foundation/TECH-9-claim-eligibility/test.md
  📄 File:    contracts/test/ClaimVault.test.ts
...
|---|---|---|
| **[unit]** | Pure TypeScript function, no real I/O | Helper/utility functions, extracted business logic |
| **[integration]** | Component boundary requiring local infrastructure | API route handlers, on-chain contract calls against a local node |
| **[e2e]** | Full pipeline — external services → chain → UI | Manual full-stack run + UI check; no automated test file |
...
|---|---|---|
| **TS Unit** | Pure TypeScript logic: extracted business rules, pure helper/utility functions | `references/ts-unit.md` |
| **Contracts** | Solidity contracts deployed on-chain | `references/contracts.md` |
| **Next.js API** | Next.js route handlers | `references/nextjs-api.md` |
...
| TypeScript unit test conventions | `references/ts-unit.md` |
