...
**What:** Draft a sprint spec at `specs/<project>/<milestone>/<TECH-N-[linear-issue-title-slug]>/spec.md` — the planning contract that gates Phase 2 implementation. The folder slug is the Linear issue title, lowercased and hyphenated (e.g. "Set Up Local PostgreSQL" → `TECH-5-set-up-local-postgresql/spec.md`).

A spec that spans multiple independent deliverables should be split into separate specs before drafting — one spec, one primary deliverable.
...
```
▶ /spec · reviewing
  📄 File:    specs/agent-insure/m1-foundation/TECH-2-initialize-repo/spec.md
...
Implement: [one sentence — file path + what it contributes. No imports, no function signatures, no parameter lists, no sub-bullets. e.g. "Create `src/lib/claims/claim-eligibility.ts` implementing `evaluateClaimEligibility` per the Core Logic diagram, and exporting the `assessClaim` function."]

Verify:
```
[exact command to run]    ← pasteable as-is; e.g. `npx vitest run claim-eligibility`
```
→ [exact expected output] ← binary pass/fail; e.g. `exits 0, all tests pass`
````
