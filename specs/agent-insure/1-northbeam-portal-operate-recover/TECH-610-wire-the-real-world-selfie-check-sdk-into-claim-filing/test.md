...
**File:**
- `server/src/routes/__tests__/world.test.js` (Integration)
...
- **POST /api/world/request**
  ...
  - [unhappy-path] a clear error is returned when the signing key isn't configured yet
...
- **Guardian completes the identity check**
  ...
  - [happy-path] once the backend confirms verification, the claim moves from "awaiting identity" to "submitted" — **not automated (needs a real device), but confirmed working by hand** on 2026-09-08 against a real Developer Portal app and a real Android phone running the World ID Sandbox app.

**Discovered while building, not anticipated in the original plan:** `@worldcoin/idkit`'s `IDKitRequestWidget` does real local (WASM) validation of `app_id`/`rp_id`/signature format and rejects synthetic test data as `generic_error` before rendering anything — confirmed by hand while diagnosing an unexpectedly-failing manual repro. So the widget's actual "ready" UI (QR/connect screen) cannot be shown by an automated test without a real Developer Portal app, not just a live device. Both that and the "moves to submitted" case are documented as manual checks (comment at the bottom of `selfie-check.spec.ts`) — now confirmed working, see spec.md's "Known dependency" section for what that took.
